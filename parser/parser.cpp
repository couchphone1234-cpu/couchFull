...
{ 
	...
	while ( not prefixOps.empty ( ) )
	{
		...
		if ( opCode == '-' )
		{
			...
			makeInstruction ( NEG_INST , s , 0 , s , scope );	
			operand = s;
		}
		//pre increment operator ###############
		else if ( opCode == INCREMENT )
			makeInstruction (  PRE_INC_INST , operand , 0 , operand , scope );
		//post operator operator ###############
		else if ( opCode == DECREMENT )
			makeInstruction (  PRE_DEC_INST , operand , 0 , operand , scope );
	}
	...
	if ( peek_token.type == '[' or peek_token.type == '.' or peek_token.type == '(' )
	{
		...
		while ( tok.type == '[' or tok.type == '.' or tok.type == '(' )
		{
			...
			else if ( tok.type == '.' )
			{
				...
				makeInstruction ( STRUCT_SELECTION , operand , expResult , 0 , scope ); //make the array index
			...
			else if ( tok.type == '(' )
			{
				...
				//make a push instuction ############
				makeInstruction ( CALL , operand , 0 , operand , scope );
